...
void ofApp::exit(){
    ...
    try {
        ...
        ofLogError() << "Error cleaning up MIDI mapper: " << e.what();
    } catch(...) {
        ofLogError() << "Unknown error cleaning up MIDI mapper";
    }
    
    try {
        // Save laser settings and cues - these are safe operations
        laser.saveSettings();
        ofLogNotice() << "Laser settings saved successfully";
    } catch(const std::exception& e) {
        ofLogError() << "Error saving laser settings: " << e.what();
    } catch(...) {
        ofLogError() << "Unknown error saving laser settings during exit, continuing...";
    }
    
    try {
        // Save cues to disk safely
        if(state) {
            saveCuesToDisk();
    ...
    try {
        // Close OSC receiver
        osc.stop();
        ofLogNotice() << "OSC receiver stopped";
    } catch(const std::exception& e) {
        ofLogError() << "Error stopping OSC receiver: " << e.what();
    } catch(...) {
        ofLogError() << "Unknown error stopping OSC receiver";
    }
    
    // Reset shared pointers safely
    try {
        state.reset();
        ofLogNotice() << "Application state cleaned up";
    } catch(...) {
        ofLogError() << "Error cleaning up application state";
    }
    
    // Note: Joystick JSON saves have been removed to prevent crashes
...
void ofApp::saveCuesToDisk(){
    try {
        ...
        bool success = ofSavePrettyJson(path, root);
        if(success) {
            ofLogNotice() << "Cues saved to " << path;
        } else {
            ofLogError() << "Failed to save cues to " << path;
        }
    } catch(const std::exception& e) {
        ofLogError() << "Exception in saveCuesToDisk: " << e.what();
    } catch(...) {
        ofLogError() << "Unknown exception in saveCuesToDisk";
    }
}
