...
void Visualiser3D ::load() {
    ...
    // Use the same filename/casing as save() to ensure cross-platform consistency
    filename ="ofxLaser/Visualiser3DLasers.json";
    bool didBackfill = false;
    auto loadAndDeserializeLasers = [&](const string& path){
        ofJson json2 = ofLoadJson(path);
        // Backfill missing visual3DParams from the currently selected preset (e.g. Default)
        try {
            if(json2.contains("lasers") && json2["lasers"].is_array()){
                Visualiser3DLaserSettings* currentPreset = visualiserLaserPresetManager.getPreset(lasersettings.getLabel());
                if(currentPreset!=nullptr){
                    for(size_t i=0; i<json2["lasers"].size(); ++i){
                        ofJson& laserJson = json2["lasers"][i];
                        if(!laserJson.contains("visual3DParams") || laserJson["visual3DParams"].empty()){
                            if(i < currentPreset->laserObjects.size()){
                                ofJson vjp;
                                ofSerialize(vjp, currentPreset->laserObjects[i].visual3DParams);
                                laserJson["visual3DParams"] = vjp;
                                didBackfill = true;
                            }
                        }
                    }
            ...
            // non-fatal: if anything goes wrong, proceed with whatever is there
        }
        lasersettings.deserialize(json2);
    };

    if(ofFile(filename).exists()) {
        loadAndDeserializeLasers(filename);
    } else {
        // Fallback: load older file name used previously (case difference)
        string legacy = "ofxLaser/visualiser3DLasers.json";
        if(ofFile(legacy).exists()) {
            loadAndDeserializeLasers(legacy);
        }
    }
    // If we upgraded/migrated missing fields, save to canonical file so next launch is consistent
    if(didBackfill){
        save();
    }

    // Prefer the selected preset on startup: if a preset exists with this label
    // and differs from the loaded run-state, apply it and persist so future restarts match.
    if(Visualiser3DLaserSettings* preset = visualiserLaserPresetManager.getPreset(lasersettings.getLabel())){
        if(lasersettings != *preset){
            lasersettings = *preset; // copies params + laserObjects from preset
            save();
        }
    }
}
