...
// Track begin/end balance to avoid calling end() after context destroyed
static bool sGuiFrameBegun = false;

string ofxLaser :: UI :: imguiSavePath;

void UI::render() {
    if(sGuiFrameBegun){
        // ImGui::Render(); handled internally by ofxImGui when ending
        imGuiOfx.end();
        sGuiFrameBegun = false;
    }
}
...
//
   ...
   // ImGui::GetStyle().Colors[ImGuiCol_ModalWindowDarkening]
    ...
    // NOTE: ofxImGui::Gui::setup() already created and set the ImGui context.
    // Creating a second context here leaks the first and causes a mismatch
    // during shutdown (double / invalid destruction -> segfault in Gui::exit()).
    // Removed redundant ImGui::CreateContext() call.
    // ImGui::CreateContext();
...
void UI::shutdown(){
    // Ensure current frame is ended before shutdown
    if(sGuiFrameBegun){
        imGuiOfx.end();
        sGuiFrameBegun = false;
    }
    // Explicitly exit gui (only if context still valid)
    // ofxImGui::Gui::exit() is idempotent regarding multiple calls due to checks inside.
    imGuiOfx.exit();
    initialised = false;
}
